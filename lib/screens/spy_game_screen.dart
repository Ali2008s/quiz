...
class _SpyGameScreenState extends State<SpyGameScreen> {
  ...
  void _startTimer() {
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      else { _timer?.cancel(); setState(() => _state = SpyState.revealed); }
    });
  }
  ...
  Widget _buildViewingState() {
    ...
    return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      _buildRoleCard(onSwipeUp: () {
        setState(() => _isWordVisible = true);
        TTSService.speak(isSpy ? 'أنت الجاسوس' : _currentPlace);
      }),
      const SizedBox(height: 20),
      if (_isWordVisible) ...[
        Text(isSpy ? 'أنت هو:\n الجاسوس 🕵️‍♂️' : 'المكان هو:\n $_currentPlace', textAlign: TextAlign.center, style: GoogleFonts.lalezar(fontSize: 28, color: const Color(0xFF1A1A2E))),
        const SizedBox(height: 10),
        GestureDetector(
          onTap: () => TTSService.speak(isSpy ? 'أنت الجاسوس' : _currentPlace),
          child: Container(
            padding: const EdgeInsets.all(10),
            decoration: BoxDecoration(color: const Color(0xFF5C6BC0), shape: BoxShape.circle, border: Border.all(color: const Color(0xFF1A1A1A), width: 2)),
            child: const Icon(Icons.volume_up_rounded, color: Colors.white, size: 28),
          )),
        const SizedBox(height: 10),
        Text('لاعب: $name', style: GoogleFonts.lalezar(fontSize: 18, color: Colors.grey)),
        const SizedBox(height: 20),
  ...
  Widget _buildRoleCard({VoidCallback? onSwipeUp}) {
    return GestureDetector(onTap: onSwipeUp, child: Container(margin: const EdgeInsets.symmetric(horizontal: 40), width: double.infinity, height: 300, decoration: BoxDecoration(color: const Color(0xFF5C6BC0), borderRadius: BorderRadius.circular(32), border: Border.all(color: const Color(0xFF1A1A1A), width: 2)),
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Image.asset('assets/images/spy.png', height: 100),
        const SizedBox(height: 10),
        Text('إكشف \n عن دورك', textAlign: TextAlign.center, style: GoogleFonts.lalezar(fontSize: 38, color: const Color(0xFF69F0AE), height: 1.1)),
        const SizedBox(height: 10),
        Text('إضغط هنا', style: GoogleFonts.lalezar(fontSize: 18, color: Colors.white70)),
        const SizedBox(height: 5),
        const Icon(Icons.touch_app, size: 36, color: Color(0xFF03A9F4)),
      ])));
  }
