...
class NotAWordGameScreen extends StatefulWidget {
  ...
  const NotAWordGameScreen({super.key, required this.rounds, required this.timePerRound, required this.team1Name, required this.team2Name, required this.categories});
...
class _NotAWordGameScreenState extends State<NotAWordGameScreen> {
  ...
  int _timeLeft = 0;
  Timer? _timer;
  String _currentPhrase = 'جاري التحميل...';
  String? _nextAIPhrase;
  ...
  Future<void> _fetchNextPhrase() async {
    if (_nextAIPhrase != null || _isLoadingAI) return;
    setState(() => _isLoadingAI = true);
    final phrase = await AIService.getNoWordTopic();
    if (mounted) {
      setState(() {
        _nextAIPhrase = phrase;
        _isLoadingAI = false;
      });
    }
  }
  ...
  Future<void> _applyNextPhrase() async {
    if (_nextAIPhrase == null) {
      setState(() => _currentPhrase = "لحظة، جاري جلب كلمة ذكية...");
      await _fetchNextPhrase();
    }
    if (mounted && _nextAIPhrase != null) {
      setState(() {
        _currentPhrase = _nextAIPhrase!;
        _nextAIPhrase = null;
      });
      TTSService.speak(_currentPhrase);
      _fetchNextPhrase(); // Pre-fetch next
    }
  }

  void _nextPhrase() => _applyNextPhrase();

  void _endTurn() {
    _timer?.cancel();
    setState(() {
      if (_currentTeam == 1) { _currentTeam = 2; _state = GameState.setup; }
      else {
        if (_currentRound < widget.rounds) { _currentRound++; _currentTeam = 1; _state = GameState.setup; }
        else _state = GameState.gameOver;
      }
    });
    _fetchNextPhrase();
  }
  ...
  void _showExitDialog() {
    showDialog(context: context, builder: (context) => CustomDialog(
      title: 'هل أنت متأكد؟', message: 'هل أنت متأكد أنك تريد الخروج من اللعبة؟',
      actions: [
        DialogButton(text: 'نعم', color: const Color(0xFFA5D6A7), onTap: () { _timer?.cancel(); Navigator.pop(context); Navigator.popUntil(context, (route) => route.isFirst); }),
  ...
  Widget build(BuildContext context) {
    return PopScope(canPop: false, onPopInvokedWithResult: (didPop, result) { if (didPop) return; _showExitDialog(); },
      child: Scaffold(backgroundColor: Colors.white, body: SafeArea(child: Column(children: [
        ...
        const CategoryHeader(title: 'ولا كلمة', imagePath: 'assets/images/silent.png', backgroundColor: Color(0xFFA5D6A7)),
  ...
  Widget _buildSetupState() {
    ...
    return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      Text('الجولة $_currentRound', style: GoogleFonts.lalezar(fontSize: 48, color: const Color(0xFF1A1A2E))),
      const SizedBox(height: 20),
      Text('دور فريق: $currentTeamName', style: GoogleFonts.lalezar(fontSize: 32, color: Colors.grey.shade700)),
      const SizedBox(height: 50),
  ...
  Widget _buildPlayingState() {
    return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        Text('$_timeLeft', style: GoogleFonts.lalezar(fontSize: 48, color: const Color(0xFFEF5350))),
      ]),
      const SizedBox(height: 30),
      Container(margin: const EdgeInsets.symmetric(horizontal: 30), padding: const EdgeInsets.symmetric(horizontal: 30, vertical: 50), width: double.infinity,
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(32), border: Border.all(color: const Color(0xFFA5D6A7), width: 4), boxShadow: const [BoxShadow(color: Colors.black12, blurRadius: 10, offset: Offset(0, 4))]),
  ...
  Widget _scoreRow(String label, int score, Color color) {
    return Container(margin: const EdgeInsets.symmetric(horizontal: 50, vertical: 10), padding: const EdgeInsets.all(15), decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFF1A1A1A), width: 3)),
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(label, style: GoogleFonts.lalezar(fontSize: 24)), Text('$score', style: GoogleFonts.lalezar(fontSize: 24))]));
  }

  Widget _actionBtn(IconData icon, Color color, VoidCallback onTap) {
    return GestureDetector(onTap: onTap, child: Container(padding: const EdgeInsets.all(15), decoration: BoxDecoration(color: color, shape: BoxShape.circle, border: Border.all(color: const Color(0xFF1A1A2E), width: 3)), child: Icon(icon, color: Colors.white, size: 36)));
  }
}
