...
class _ConfessionsGameScreenState extends State<ConfessionsGameScreen> with TickerProviderStateMixin {
  ...
  Future<void> _fetchNextQuestion() async {
    ...
    final q = await AIService.getConfession();
    if (mounted) {
      setState(() {
        _nextAIQuestion = q;
        _isLoadingAI = false;
  ...
  void _onWheelStop() async {
    ...
    if (_nextAIQuestion == null) {
      setState(() {
        _currentQuestion = "لحظة، جاري جلب سؤال اعتراف ذكي...";
  ...
  Widget build(BuildContext context) {
    return PopScope(canPop: false, onPopInvokedWithResult: (didPop, result) { if (didPop) return; _showExitDialog(); },
      child: Scaffold(backgroundColor: Colors.white, body: SafeArea(child: Column(children: [
        ...
        const CategoryHeader(title: 'إعترافات', imagePath: 'assets/images/fingerprint.png', backgroundColor: Color(0xFFFFD54F)),
  ...
  Widget _buildMainContent() {
    switch (_state) {
      ...
      case GameState.fingerprint: return _buildFingerprintState();
    }
  }
    ...
    return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      Stack(alignment: Alignment.topCenter, children: [
        AnimatedBuilder(animation: _wheelAnimation, builder: (context, child) {
          return Transform.rotate(angle: _wheelAnimation.value, child: CustomPaint(size: const Size(300, 300), painter: WheelPainter(players: widget.players)));
        }),
        Transform.translate(offset: const Offset(0, -10), child: const Icon(Icons.location_on, color: Color(0xFFEF5350), size: 40)),
      ]),
  ...
  Widget _buildQuestionState() {
    return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      const SizedBox(height: 20),
      Container(padding: const EdgeInsets.symmetric(horizontal: 40, vertical: 10), decoration: BoxDecoration(color: const Color(0xFFFFD54F), borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFF1A1A1A), width: 3)), child: Text(_selectedPlayer, style: GoogleFonts.lalezar(fontSize: 24, color: const Color(0xFF1A1A2E)))),
      const SizedBox(height: 30),
      Container(margin: const EdgeInsets.symmetric(horizontal: 30), padding: const EdgeInsets.all(30), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(24), border: Border.all(color: const Color(0xFF1A1A1A), width: 4)),
    ...
    return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      GestureDetector(onLongPressEnd: (_) {
        showDialog(context: context, builder: (context) => CustomDialog(
          ...
          actions: [DialogButton(text: 'موافق', color: isTrue ? const Color(0xFFA5D6A7) : const Color(0xFFEF5350), onTap: () { Navigator.pop(context); setState(() => _state = GameState.wheel); })],
        ));
      }, child: Column(children: [
        Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: const Color(0xFF1A1A1A), width: 3)), child: Image.asset('assets/images/fingerprint.png', height: 120)),
        const SizedBox(height: 20),
        Text('ضع إصبعك مطولاً', style: GoogleFonts.lalezar(fontSize: 24, color: Colors.grey)),
      ])),
      const SizedBox(height: 30),
    ]);
  }
...
class WheelPainter extends CustomPainter {
  ...
  void _drawText(Canvas canvas, String text, Offset center, double radius, double angle) {
    ...
    final textPainter = TextPainter(text: TextSpan(text: text, style: GoogleFonts.lalezar(fontSize: 16, color: const Color(0xFF1A1A2E), fontWeight: FontWeight.bold)), textDirection: TextDirection.rtl)..layout();
    textPainter.paint(canvas, Offset(radius * 0.4, -textPainter.height / 2)); canvas.restore();
  }
